...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
  ...
  if(start > MAX_MAP_LOCATION || end > MAX_MAP_LOCATION || start < MIN_MAP_LOCATION || end < MIN_MAP_LOCATION){
    return 0;
  }
  //We also handle if the start == the end.
  if(start == end){
    type[0] = ROAD;
    type[1] = RAIL;
    type[2] = BOAT;
...
int edgePriority(LocationID source, LocationID destination, Map g){
  ...
  if(connections(g, source, destination, &transportTypesAvailable[0]) > 0){
    return 1;
  }
  return 0;
}
...
int findPath(Map gameMap, LocationID src, LocationID dest, int max, int *path)
{
  assert(gameMap != NULL);
  Queue openList = newQueue();
  LocationID* pred = malloc(sizeof(LocationID)*numV(gameMap));
  int* dist = malloc(sizeof(int)*numV(gameMap));
  LocationID cur;
  int i;
  LocationID v;
  LocationID t;
  ...
  for (v = 0; v < numV(gameMap); v++){
    pred[v] = -1;
    dist[v] = 1000000000;
    enterQueue(openList,v);
  }
  dist[dest] = 0;
  while (!emptyQueue(openList)) {
    s = leaveQueue(openList);
    for (t = 0; t < numV(gameMap); t++) {
      len = edgePriority(s, t, gameMap);
      //len = gameMap->edgameMapes[s][t];
      if (len == 0) continue;
      if (dist[s]+len < dist[t] && len < max) {
	pred[t] = s;
	dist[t] = dist[s]+len;
	enterQueue(openList, t);
      }
    }
  }
  /*
    printf("pred:\n");
    for(i = 0; i < numV(gameMap); i++){
    printf("\t%d: %d\n", i, pred[i]);
    }
